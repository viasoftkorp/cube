...
pub struct QueryResult {
    ...
    pub row_count: usize,
    pub data: Vec<Vec<DBResponseValue>>,
}
...
impl QueryResult {
    ...
    #[inline]
    pub fn column(&self, idx: usize) -> &[DBResponseValue] {
        &self.data[idx]
    }

    #[inline]
    pub fn cell(&self, row: usize, col: usize) -> &DBResponseValue {
        &self.data[col][row]
    }
    ...
    pub fn from_js_raw_data(js_raw_data: JsRawColumnarData) -> Result<Self, ParseError> {
        ...
        // JS already gives us column-major data; wrap each primitive into
        // `DBResponseValue` per column without re-pivoting.
        let data: Vec<Vec<DBResponseValue>> = columns
            .into_iter()
            .map(|col| col.into_iter().map(DBResponseValue::Primitive).collect())
            .collect();
