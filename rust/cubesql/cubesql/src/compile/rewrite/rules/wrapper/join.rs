...
impl WrapperRules {
    ...
    /// deliberately admitted: a segment member on a raw side is a row filter shipped
    /// in the side's own load query (`request.segments`) — part of which rows the side
    /// returns, not a column that needs materializing — and a segment in the
    /// projection goes through the wrapper's pre-existing segment handling (a typed
    /// NULL literal), never an unresolvable column reference. When the members can't
    /// be inspected, assume the worst.
