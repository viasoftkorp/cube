...
fn modify_sql_ast_many(
    ...
    std::thread::scope(|scope| {
        rewrite_thread()
            .spawn_scoped(scope, || modify_parsed_query(sql, actions, ctx, reported))
            .map_err(|e| {
                DataFusionError::Internal(format!("Failed to start the rewrite thread: {}", e))
            })?
            .join()
...
/// The awaitable rewrite the endpoints use: the work runs on a thread of its
/// own and the tokio worker is not parked for it. A rewrite that fails is the
/// caller's; a thread that cannot start, or one that panics, is this API's.
async fn rewrite_sql(
    ...
    rewrite_thread()
        .spawn(move || {
            // Nothing to do if the request was dropped before the result
            let _ = sender.send(modify_parsed_query(&sql, &actions, &meta, &reported));
        })
        .map_err(|e| CubeError::internal(format!("Failed to start the rewrite thread: {}", e)))?;
    receiver
        .await
        .map_err(|_| CubeError::internal("The rewrite thread ended without a result".to_string()))?
        .map_err(|e| CubeError::user(e.to_string()))
}

/// Upper bound on the SQL text a request may carry. The renderer and the
/// drop recurse once per conjunct wherever in the query it sits, so the
/// recursion is bounded through the text that produces it.
const MAX_SQL_BYTES: usize = 4 << 20;

/// The stack a rewrite runs on. Measured: 20k left-deep conjuncts overflow
/// 2 MiB, so a conjunct costs about 100 bytes of stack in a debug build;
/// [`MAX_SQL_BYTES`] of the shortest conjuncts is some 500k of them.
const REWRITE_STACK_BYTES: usize = 256 << 20;

/// Upper bound on the operators a query may hold; a conjunct `a = 1` is two.
/// Planning recurses once per operator: measured in a debug build on the
/// rewrite stack, 5000 conjuncts planned and 10000 overflowed.
const MAX_SQL_OPERATORS: usize = 4000;

/// Refuses SQL text larger, or holding more operators, than the rewrite and
/// the planning are sized for.
fn assert_sql_size(sql: &str) -> Result<(), CubeError> {
    if sql.len() > MAX_SQL_BYTES {
        return Err(CubeError::user(format!(
            "The query is {} bytes, more than the {} supported",
            sql.len(),
            MAX_SQL_BYTES
        )));
    }
    let operators = count_sql_operators(sql);
    if operators > MAX_SQL_OPERATORS {
        return Err(CubeError::user(format!(
            "The query has {} operators, more than the {} supported",
            operators, MAX_SQL_OPERATORS
        )));
    }
    Ok(())
}

/// Counts the binary operators of a SQL text - `AND`, `OR` and the symbolic
/// ones - outside its string literals, which is what a left-deep chain and so
/// the planner's recursion is made of. Anything else is not counted.
fn count_sql_operators(sql: &str) -> usize {
    let mut count = 0;
    ...
    for c in sql.chars() {
        if in_literal {
            // A doubled quote stays inside the literal, which a later quote
            // then closes; counting nothing in between is right either way
            in_literal = c != '\'';
            continue;
        }
        if c == '\'' {
...
/// A thread for the rewrite, with [`REWRITE_STACK_BYTES`] of stack: a tokio
/// worker's 2 MiB is one the render of a long query overflows, and an
/// overflow aborts the process.
fn rewrite_thread() -> std::thread::Builder {
    std::thread::Builder::new()
        .name("sql-filters-rewrite".to_string())
        .stack_size(REWRITE_STACK_BYTES)
}
...
/// Walked iteratively, so that a deeply nested one costs no stack.
...
/// parentheses, which are the shapes the renderer recurses through and so
/// what the bound is about. Iterative, so that counting can't overflow the
/// stack it is there to protect.
...
async fn plan_and_extract_filters(
    ...
    // Planning recurses per operator, so it runs on the rewrite stack rather
    // than the worker's, driven from there by the runtime it was called on
    ...
    rewrite_thread()
        .spawn(move || {
            let planned = runtime.block_on(async {
                // A planner fault of its own stays internal, so the gateway
                // answers it as one; a query that does not plan is the caller's
                ...
                // A statement that compiles but has no plan - SET, SHOW, BEGIN -
                // is the caller's, like the parse error `add` gives the same input
                let logical_plan = query_plan.try_as_logical_plan().map_err(|_| {
                    CubeError::user("Only SELECT queries are supported".to_string())
                })?;
        ...
        .map_err(|e| CubeError::internal(format!("Failed to start the planning thread: {}", e)))?;
    receiver.await.map_err(|_| {
        CubeError::internal("The planning thread ended without a result".to_string())
    })?
}
...
mod tests {
    ...
    /// A clause at the bound is rewritten and rendered from a caller with a
    /// tokio worker's 2 MiB of stack; the rewrite runs on a stack of its own.
    /// The addition is what brings the clause to the bound: one more is refused.
    #[test]
    fn test_clause_at_the_bound_renders_on_a_worker_stack() {
        let rendered_len = std::thread::Builder::new()
            .stack_size(2 << 20)
            .spawn(|| {
                let ctx = get_test_tenant_ctx();
                let mut clause = String::from("1 = 1");
                for i in 2..MAX_CLAUSE_PREDICATES {
                    clause.push_str(&format!(" AND customer_gender = 'v{}'", i));
                }
                let sql = format!(
                    "SELECT customer_gender FROM KibanaSampleDataEcommerce WHERE {} GROUP BY 1",
                ...
                // Parses, appends and renders: the whole path a request takes
                // before anything is planned
                let (built, applied) =
                    modify_sql_ast(&sql, &ModifyAction::Add(filter.clone()), &ctx).unwrap();
                assert!(applied);
                // and one past it is refused, by the count carried across the
                // batch rather than a recount
                let another = V1LoadRequestQueryFilterItem {
                    values: Some(vec!["y".to_string()]),
                    ..filter
                };
                let err = modify_sql_ast(&built, &ModifyAction::Add(another), &ctx).unwrap_err();
                assert!(err.to_string().contains("more than the"), "{}", err);
                assert!(
                    built.ends_with("\"notes\" = 'x' GROUP BY 1"),
                    "{}",
                    &built[built.len() - 60..]
                );
                built.len()
            ...
            .unwrap()
            .join()
            .expect("a clause at the bound renders and drops without overflowing");
    ...
    /// The recursion of the renderer is not only in the outermost clauses: a
    /// long chain in a CTE, which no clause bound counts, has to render too.
    /// The rewrite runs on its own stack, so a caller with a worker's 2 MiB
    /// is not what it depends on.
    #[test]
    fn test_long_chain_outside_the_outermost_select_renders() {
        std::thread::Builder::new()
            .stack_size(2 << 20)
            .spawn(|| {
                let ctx = get_test_tenant_ctx();
                ...
                let sql = format!(
                    "WITH c AS (SELECT customer_gender FROM KibanaSampleDataEcommerce WHERE {}) \
                     SELECT customer_gender FROM c",
                    clause
                );
                let (built, applied) =
                    modify_sql_ast_many(&sql, &[], &ctx, &ReportedFilters::none()).unwrap();
                assert!(applied.is_empty());
                assert!(built.ends_with("SELECT customer_gender FROM c"));
            })
            .unwrap()
            .join()
            .expect("a long chain in a CTE renders without overflowing");
    }

    /// SQL text past the size the rewrite stack is measured for is refused
    /// before it is parsed.
    #[tokio::test]
    async fn test_oversized_sql_is_refused() -> Result<(), CubeError> {
        use crate::compile::{test::get_test_session, DatabaseProtocol};
        ...
        let sql = format!(
            "SELECT customer_gender FROM KibanaSampleDataEcommerce WHERE notes = '{}'",
            "x".repeat(MAX_SQL_BYTES)
        );
        let err = get_sql_filters(&sql, meta, session)
            .await
            .expect_err("oversized SQL is refused");
        assert!(err.message.contains("more than the"), "{}", err.message);

        Ok(())
    }

    /// Planning recurses per operator, and a worker's 2 MiB of stack is not
    /// what it gets to run on: a chain that overflows a worker plans, or is
    /// refused by the rewrite engine, without taking the process down.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_planning_runs_off_the_worker_stack() {
        use crate::compile::{test::get_test_session, DatabaseProtocol};

        let meta = get_test_tenant_ctx();
        let session = get_test_session(DatabaseProtocol::PostgreSQL, meta.clone()).await;
        // Three times what a worker's stack survives, and under the
        // operator bound: a conjunct is two operators
        let mut clause = String::from("1 = 1");
        for i in 0..1_500 {
            clause.push_str(&format!(" AND customer_gender = 'v{}'", i));
        }
        ...
        // Called from a thread with a worker's stack, as the endpoint is
        let runtime = tokio::runtime::Handle::current();
        let outcome = std::thread::Builder::new()
            .stack_size(2 << 20)
            .spawn(move || runtime.block_on(get_sql_filters(&sql, meta, session)))
            .unwrap()
            .join()
            .expect("planning a long chain does not overflow the caller's stack");
        if let Err(err) = outcome {
            assert!(
                err.message.contains("Failed to plan the query"),
                "unexpected error: {}",
                err.message
            );
        }
    ...
    /// A query with more operators than planning is sized for is refused
    /// before it is parsed, wherever in the query the operators sit.
    #[tokio::test]
    async fn test_too_many_operators_are_refused() -> Result<(), CubeError> {
        use crate::compile::{test::get_test_session, DatabaseProtocol};

        let meta = get_test_tenant_ctx();
        let session = get_test_session(DatabaseProtocol::PostgreSQL, meta.clone()).await;
        let terms = (0..=MAX_SQL_OPERATORS + 1)
            .map(|_| "1")
            .collect::<Vec<_>>()
            .join(" + ");
        let sql = format!(
            "WITH c AS (SELECT {} AS n FROM KibanaSampleDataEcommerce) SELECT n FROM c",
            terms
        );

        let err = get_sql_filters(&sql, meta, session)
            .await
            .expect_err("too many operators are refused");
        assert!(err.message.contains("operators"), "{}", err.message);

        Ok(())
    }

    /// Operators are counted outside string literals, so a query full of
    /// dates is not refused for the dashes in them.
    #[test]
    fn test_sql_operators_are_counted_outside_literals() {
        assert_eq!(
            count_sql_operators("SELECT a FROM t WHERE d >= '2024-01-01' AND d <= '2024-12-31'"),
            3
        );
        assert_eq!(
            count_sql_operators("SELECT a || 'x -- y' FROM t WHERE a <> 'it''s'"),
            2
        );
        assert_eq!(count_sql_operators("SELECT 1 + 1 + 1"), 2);
        assert_eq!(
            count_sql_operators("select a from t where a = 1 or b = 2"),
            3
        );
    }
}
