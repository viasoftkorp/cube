...
fn clear_outermost_filters(sql: &str) -> DFResult<String> {
    let mut query = parse_single_query(sql)?;
...
fn is_leaf_filter_predicate(
    ...
    let is_member_ref =
        |expr: &ast::Expr| is_member_column_ref(filter_column_expr(expr), relations, 0, budget);
    ...
    match expr {
        ast::Expr::BinaryOp { left, op, right } => match op {
            ...
            | ast::BinaryOperator::GtEq => {
                ...
                (is_member_ref(left) && is_filter_value(right))
                    || (is_filter_value(left) && is_member_ref(right))
            }
            _ => false,
        },
        ...
        ast::Expr::Between {
            expr, low, high, ..
        } => is_member_ref(expr) && is_filter_value(low) && is_filter_value(high),
        _ => false,
    }
}
...
/// The functions whose value the engine works out when it plans the query,
/// and which a filter can be written against. A query saying
/// `CURRENT_DATE - INTERVAL '7 days'` is reported as the date that came out
/// of it, so the SQL and the filter read back do not spell the same value.
...
/// Whether the expression is a date the engine computes when it plans: built
/// only of literals, intervals and the functions above, with no column
/// reference anywhere in it. The whole expression is walked, iteratively, so
/// that a deeply nested one costs no stack.
fn is_planning_time_date(expr: &ast::Expr) -> bool {
    ...
    while let Some(expr) = pending.pop() {
        visited += 1;
        if visited > MAX_CLAUSE_PREDICATES {
            return false;
        }
...
fn apply_action_to_outermost_select(
    ...
    let match_ctx = |is_reported| MatchContext {
        is_reported,
    ...
    match action {
        ModifyAction::Add(filter) => {
            ...
            Ok(add_expr_to_clause(
                clause_mut(select, kind),
                expr,
                match_ctx(false),
            ))
        }
        ModifyAction::Remove(filter) => {
            ...
            let ctx = match_ctx(reported.contains(&filter_key(filter)));
            Ok(remove_expr_from_clause(clause_mut(select, kind), &expr, ctx) > 0)
        }
        ModifyAction::Replace { old, new } => {
            ...
            let ctx = match_ctx(reported.contains(&filter_key(old)));
            if old_kind == new_kind {
                // Replace in place, preserving positions within the clause
                Ok(
                    replace_expr_in_clause(clause_mut(select, old_kind), &old_expr, new_expr, ctx)
                        > 0,
                )
            } else {
                // The filters belong to different clauses: remove, then add
                if remove_expr_from_clause(clause_mut(select, old_kind), &old_expr, ctx) == 0 {
                    return Ok(false);
                }
                add_expr_to_clause(clause_mut(select, new_kind), new_expr, ctx);
                Ok(true)
            }
        }
    }
...
fn assert_clauses_are_bounded(select: &ast::Select) -> DFResult<()> {
    for clause in
...
fn add_expr_to_clause(
    ...
    if clause_contains_expr(&clause, &expr, ctx) {
...
#[derive(Debug, Clone, Copy)]
struct MatchContext {
    /// Whether the query reports the filter being matched. A filter it does
    /// report but does not spell out the way this API writes it - a range the
    /// engine merged, a date it computed - is matched by the column it stands
    /// on, which a filter that isn't there must never be.
    is_reported: bool,
    /// Whether a column written without its relation name is the same column.
    /// It is when the outermost FROM holds a single relation, as there is no
    /// other one a bare column could belong to.
    ignore_qualifier: bool,
}

impl MatchContext {
    fn key(&self, expr: &ast::Expr) -> FilterKey {
        expr_key(expr, self.ignore_qualifier)
    }
}

/// A comparison key for a filter expression: its SQL rendering with quoted
/// literals reduced to a common form. The planner reports a date as
/// `2020-01-01T00:00:00.000Z` however the query wrote it, so a filter read
/// back from a plan has to match the literal it came from. Identifier quoting
/// is reduced as well: this API writes a column as `orders_view."status"`
/// while a query is free to write it as `orders_view.status`, and in
/// PostgreSQL those name the same column.
/// The comparison key, optionally with the relation name of every qualified
/// column dropped. A query is free to write a column without the relation it
/// belongs to, while this API always writes the relation, and where the
/// outermost FROM holds a single relation both name the same column.
fn expr_key(expr: &ast::Expr, drop_qualifiers: bool) -> String {
    ...
    while let Some(c) = chars.next() {
        match c {
            '\'' => {
                ...
                key.push('\'');
                key.push_str(&normalize_filter_value(&literal).replace('\'', "''"));
                key.push('\'');
            }
            '"' => {
                push_word(&mut key, &mut word, Some('"'));
...
/// Flushes a run of unquoted characters into the key. PostgreSQL folds an
/// unquoted identifier to lower case; `ILIKE` is the same filter as `LIKE`
/// once it is a Cube filter, which knows no case sensitivity; and a measure
/// filter is the same one whether the query aggregates it with `MEASURE()` or
/// with the aggregation the measure is defined with, which is what this API
/// writes.
...
/// Whether an identifier means the same thing written without its quotes.
/// PostgreSQL folds an unquoted identifier to lower case, so only one that is
/// already lower case, and holds nothing that would end it, keeps its meaning.
...
fn clause_contains_expr(clause: &ast::Expr, needle: &ast::Expr, ctx: MatchContext) -> bool {
    let keys = all_conjuncts(clause)
        .into_iter()
        .map(|expr| ctx.key(expr))
        .collect::<Vec<_>>();

    if keys.contains(&ctx.key(needle)) {
        return true;
    }
...
fn matching_conjuncts(
    ...
    needle: &ast::Expr,
    ctx: MatchContext,
) -> Result<Vec<ast::Expr>, ast::Expr> {
    let needle_key = ctx.key(needle);

    if and_conjuncts_exact(&clause)
        .into_iter()
        .any(|conjunct| ctx.key(conjunct) == needle_key)
    {
...
fn remove_expr_from_clause(
    ...
    let clause = match matching_conjuncts(clause, needle, ctx) {
        Ok(conjuncts) => {
            let needle_key = ctx.key(needle);
            let before = conjuncts.len();
            let kept = conjuncts
                .into_iter()
                .filter(|conjunct| ctx.key(conjunct) != needle_key)
                .collect::<Vec<_>>();
...
/// Removes the predicates on the column the needle filters, for a filter the
/// query does report but whose predicates it does not spell out the way this
/// API writes them.
///
/// The engine reports what it planned, not what was written: two bounds on a
/// time member come back as one range, a bound written with `<` comes back as
/// `<=` a millisecond earlier, and `CURRENT_DATE - INTERVAL '7 days'` comes
/// back as the date it worked out. None of those can be matched against the
/// query by value, so the filter is matched by the column it stands on, and
/// every predicate on that column goes with it, which is what a merged range
/// needs.
///
/// This only applies to a filter the query currently reports, so that
/// deleting a filter that isn't there stays the no-op it is meant to be
/// rather than taking an unrelated predicate on the same column with it.
fn remove_reported_column_predicates(
    ...
    let needle_column = filter_column_key(needle, ctx).filter(|_| ctx.is_reported);
...
fn replace_expr_in_clause(
    ...
    let clause = match matching_conjuncts(clause, old, ctx) {
        Ok(conjuncts) => {
    ...
    *option_clause = Some(clause);
    ...
    add_expr_to_clause(option_clause, new, ctx);
    removed
}

/// Operators whose values are dates, and so are reported in the form the
/// query wrote them rather than the one the planner canonicalizes them to.
const DATE_OPERATORS: [&str; 6] = [
    "inDateRange",
    "notInDateRange",
    "beforeDate",
    "beforeOrOnDate",
...
/// Reduces the values of a date filter back to the form they were written in.
/// The planner canonicalizes a date to `2020-01-01T00:00:00.000Z` whatever the
/// query said, and reporting that back would hand callers a value their own
/// query doesn't contain.
fn report_filter(mut filter: V1LoadRequestQueryFilterItem) -> V1LoadRequestQueryFilterItem {
    if let Some(items) = filter.and.take() {
        filter.and = Some(report_group_items(items));
        return filter;
    }
    if let Some(items) = filter.or.take() {
        filter.or = Some(report_group_items(items));
        return filter;
    }

    let is_date_filter = filter
        .operator
        .as_deref()
        .is_some_and(|operator| DATE_OPERATORS.contains(&operator));
    if !is_date_filter {
        return filter;
    }
...
/// A date whose time part is zero is reported as the date alone, which is how
/// a query writes it. A value carrying a real time is reported as it stands,
/// down to its trailing zeros, since shortening it would report something the
/// query doesn't say either.
fn report_date_value(value: &str) -> String {
    if !is_iso_date_like(value) {
        return value.to_string();
    }

    let date = strip_any_suffix(value, &["Z", "+00:00", "+0000"]);
    let date = strip_any_suffix(date, &[".000"]);
    let date = strip_any_suffix(date, &["T00:00:00", " 00:00:00"]);
...
fn report_group_items(items: Vec<serde_json::Value>) -> Vec<serde_json::Value> {
    items
        .into_iter()
        .map(
            |item| match serde_json::from_value::<V1LoadRequestQueryFilterItem>(item.clone()) {
                Ok(item_filter) => serde_json::to_value(report_filter(item_filter)).unwrap_or(item),
                Err(_) => item,
            },
        )
        .collect()
            ...
            if let LogicalPlan::Extension(ext) = plan {
                if let Some(scan_node) = ext.node.as_any().downcast_ref::<CubeScanNode>() {
                    if let Some(filters) = &scan_node.request.filters {
                        self.0.extend(filters.iter().cloned().map(report_filter));
                    }
                    for time_dimension in scan_node.request.time_dimensions.iter().flatten() {
                        ...
                        self.0.push(report_filter(V1LoadRequestQueryFilterItem {
...
/// The keys of the filters a query reports, as the modification path
/// compares them.
fn reported_keys(filters: Vec<V1LoadRequestQueryFilterItem>) -> HashSet<FilterKey> {
    filters.iter().map(filter_key).collect()
}
...
fn assert_filter_count(filters: &[V1LoadRequestQueryFilterItem]) -> Result<(), CubeError> {
...
async fn add_filters_and_verify(
    ...
    let (new_sql, _) = modify_sql_ast_many(&sql, &actions, &meta, &HashSet::new())
...
pub async fn delete_sql_filters(
    ...
    let actions = filters
        ...
        .collect::<Vec<_>>();
    let (new_sql, _) = modify_sql_ast_many(sql, &actions, &meta, &reported)
        .map_err(|e| CubeError::user(e.to_string()))?;
...
pub async fn replace_sql_filters(
    ...
    let (new_sql, applied) = modify_sql_ast_many(sql, &actions, &meta, &reported)
        .map_err(|e| CubeError::user(e.to_string()))?;
...
mod tests {
    ...
    fn modify_sql_ast(
        ...
        let (sql, applied) =
            modify_sql_ast_many(sql, slice::from_ref(action), ctx, &HashSet::new())?;
        Ok((sql, applied[0]))
    }
    ...
    #[test]
    fn test_clear_outermost_filters_keeps_non_filter_predicates() -> DFResult<()> {
    ...
    #[tokio::test]
    async fn test_set_sql_filters_keeps_non_filter_predicate() -> Result<(), CubeError> {
        use crate::compile::{test::get_test_session, DatabaseProtocol};
    ...
    #[test]
    fn test_clear_outermost_filters_keeps_computed_derived_column() -> DFResult<()> {
    ...
    fn test_modify_sql_ast_large_clause() -> DFResult<()> {
        ...
        let actions = (0..MAX_FILTERS)
            ...
            .collect::<Vec<_>>();
        let (built, applied) = modify_sql_ast_many(sql, &actions, &ctx, &HashSet::new())?;
    ...
    async fn test_sql_filters_round_trip_reported_values() -> Result<(), CubeError> {
        ...
        assert!(
            values[0].starts_with("2020-01-01T12:30:00")
                && values[1].starts_with("2021-01-01T23:59:59.999"),
