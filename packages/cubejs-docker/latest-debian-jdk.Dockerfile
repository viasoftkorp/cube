...
RUN apt-get update \
    ...
    && apt-get install -y python3 python3.13 libpython3.13-dev gcc g++ make cmake openjdk-17-jdk-headless \
...
RUN groupadd cube && useradd -ms /bin/bash -g cube cube \
    ...
    && apt-get update \
    && apt-get install -y --no-install-recommends libssl3 openjdk-17-jre-headless python3.13 libpython3.13-dev \
    && rm -rf /var/lib/apt/lists/* \
    && mkdir cube \
